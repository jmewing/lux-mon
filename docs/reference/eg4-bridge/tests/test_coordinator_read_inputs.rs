...
async fn read_inputs_sends_packet() {
    ...
    tokio::spawn(async move {
        ...
        channels_clone.from_inverter.send(eg4::inverter::ChannelData::Packet(response_clone)).unwrap();
    });
    ...
    match receiver.recv().await.unwrap() {
        eg4::inverter::ChannelData::Packet(packet) => {
            match packet {
                Packet::TranslatedData(td) => {
                    assert_eq!(td.datalog, inverter.datalog().unwrap());
                    assert_eq!(td.device_function, DeviceFunction::ReadInput);
                    assert_eq!(td.inverter, inverter.serial().unwrap());
