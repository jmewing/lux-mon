...
#   feat(skill): ...     -> MAJOR  (explicit opt-in; only way to bump major)
#   feat(inverter): ...  -> MINOR  (new inverter support)
#   feat: ...            -> MINOR  (new feature)
#   feat(scope): ...     -> MINOR  (new feature, any scope)
...
if [[ -z "$FORCE_BUMP" ]]; then
  ...
  while IFS= read -r subject; do
    ...
    # feat (plain or any other scope) -> minor (new feature)
